...
static void test_path(struct __test_metadata *_metadata, const char *path, int ret)
{
	int fd;
	struct stat statbuf;

	ASSERT_EQ(ret, access(path, R_OK | X_OK));
	ASSERT_EQ(ret, stat(path, &statbuf));
...
static void enforce_map(struct __test_metadata *_metadata, int map, bool subpath)
{
	...
	fd_pick = bpf_load_program(BPF_PROG_TYPE_LANDLOCK_HOOK,
			(const struct bpf_insn *)&prog_pick,
			sizeof(prog_pick) /
			sizeof(struct bpf_insn), "GPL", 0, log,
			sizeof(log), &subtype);
	ASSERT_NE(-1, fd_pick) {
		TH_LOG("Failed to load fs_pick program: %s\n%s",
				strerror(errno), log);
