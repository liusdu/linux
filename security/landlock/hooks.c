		...
		struct landlock_chain *chain)
{
	...
	case LANDLOCK_HOOK_FS_GET:
		return landlock_save_ctx_fs_get(hook_ctx->fs_get, chain);
	}
	WARN_ON(1);
	return 1;
}
...
/**
 * landlock_event_deny - run Landlock programs tied to an event
 *
 ...
 */
static bool landlock_event_deny(enum landlock_hook_type hook_type,
		struct landlock_hook_ctx *hook_ctx,
		...
		struct landlock_hook_ctx *hook_ctx, u64 triggers)
{
	...
	deny = landlock_event_deny(hook_type, hook_ctx,
