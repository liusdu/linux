...
static bool landlock_progs_are_subset(const struct landlock_prog_set *parent,
...
static bool landlock_task_has_subset_progs(const struct task_struct *parent,
		const struct task_struct *child)
{
#ifdef CONFIG_SECCOMP_FILTER
	if (landlock_progs_are_subset(parent->seccomp.landlock_prog_set,
		...
		const struct task_struct *child)
{
	...
	if (landlock_task_has_subset_progs(parent, child))
		return 0;
