		...
		const struct bpf_prog_extra *prog_extra)
{
	...
	case LANDLOCK_HOOK_FS_GET:
		// FIXME: for fs_get to not be alone (must be previous of an fs_pick)
 ...
 *
 * Keep in sync with common.c:landlock_is_forkable().
 */
...
static bool bpf_landlock_is_valid_subtype(struct bpf_prog_extra *prog_extra)
{
	...
	if (subtype->landlock_hook.options & LANDLOCK_OPTION_PREVIOUS) {
		...
		if (IS_ERR(previous))
			return false;
		if (!good_previous_prog(subtype->landlock_hook.type, previous)) {
		...
		const struct bpf_prog_extra *prog_extra)
{
	...
	/* generic functions */
	// TODO: do we need/want update/delete functions for every LL prog? => impurity vs. audit
	switch (func_id) {
